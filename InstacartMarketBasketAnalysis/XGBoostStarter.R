...
ordert <- fread(file.path(dataFolder,'order_products__train.csv'));
orderp <- fread(file.path(dataFolder,'order_products__prior.csv'));
...
test$reordered <- (test$reordered > 0.25) * 1
...
write.csv(submission, file = file.path(dataFolder,"submit3.csv"), row.names = F)
